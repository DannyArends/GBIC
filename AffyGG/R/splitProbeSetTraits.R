...
splitProbeSetTraits <- function( affy.signals, directory )
{
     ...
     for( idxProbeSet in 1:length( listProbeSets ) )
     {
        ...
        nProbes    <- nrow( affy.signals[ affy.signals$probeset == 
                                          listProbeSets[ idxProbeSet ] , ] )
        traitsTemp <- data.frame( affy.signals[ affy.signals$probeset == 
                                  listProbeSets[ idxProbeSet ] , ][,-1],
